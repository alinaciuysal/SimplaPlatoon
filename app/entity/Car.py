...
class Car:
    ...
    def addToSimulation(self, tick):
        ...
        try:
            ...
            # NEW: car-following model
            # default model name for car-following model,
            # others are here http://sumo.dlr.de/wiki/Definition_of_Vehicles,_Vehicle_Types,_and_Routes#Car-Following_Models
            # self.type = "Krauss"

            # traci.vehicle.setParameter(self.id, "carFollowModel", self.type)
            # param = traci.vehicle.getParameter(self.id, "carFollowModel")
            # print(param)
            # def getParameter(self, objID, param):
            # type = traci.vehicle.getTypeID(self.id)
            # print(type)

            # traci.vehicle.setType(self.id, self.type)
